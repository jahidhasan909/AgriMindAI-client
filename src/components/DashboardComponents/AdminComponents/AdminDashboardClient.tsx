...
const AdminDashboardClient: React.FC<DashboardClientProps> = ({ stats, payments, products }) => {
    ...
    return (
        <div className="min-h-screen  dark:bg-slate-950 p-4 md:p-8 font-sans antialiased text-slate-900 dark:text-slate-100">
            <div className="max-w-[1600px] mx-auto space-y-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button className="inline-flex items-center gap-2 text-sm font-semibold bg-[#79a603] text-white hover:bg-[#688f02] px-4 py-2.5 rounded-xl transition-all shadow-xs shadow-[#79a603]/10 cursor-pointer">
                            <Plus className="h-4 w-4" /> Add Product
                        </button>
                        <button className="inline-flex items-center gap-2 text-sm font-semibold bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:bg-slate-50 text-slate-700 dark:text-slate-300 px-4 py-2.5 rounded-xl transition-all cursor-pointer">
                            <Download className="h-4 w-4" /> Import Data
                        </button>
                    </div>
                </div>

                {/* Grid 1: Metric Overview Cards */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    
                    {/* Active Highlight Card - Total Users */}
                    <Card className="p-6 bg-[#79a603] text-white border-none rounded-3xl relative overflow-hidden shadow-lg shadow-[#79a603]/10 group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 transform translate-x-2 -translate-y-2">
                            <Users className="h-32 w-32" />
                        </div>
                        <div className="flex justify-between items-start">
                            <span className="text-sm font-semibold text-white/90">Total Framework Users</span>
                        ...
                        <div className="mt-6 space-y-1">
                            ...
                            <p className="text-xs text-white/80 font-medium">+14% increased from last month</p>
                        </div>
                    </Card>
                        ...
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex items-center gap-4 text-xs font-semibold">
                                <span className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300">
                                    <span className="h-2 w-2 rounded-full bg-[#79a603]"></span> Gross Revenue
                                </span>
                                <span className="flex items-center gap-1.5 text-slate-400">
                                    <span className="h-2 w-2 rounded-full bg-slate-200 dark:bg-slate-700"></span> Target Listings
                                </span>
                            </div>
                        </div>

                        {/* Minimalist Grid BarChart */}
                    ...
                    <Card className="p-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl shadow-xs flex flex-col justify-between">
                        <div>
                            <div className="flex justify-between items-start">
                                ...
                                <span className="p-2.5 bg-[#79a603]/10 text-[#79a603] rounded-xl">
                                    <DollarSign className="h-4 w-4" />
                                </span>
                            </div>
                            <div className="mt-6">
                                <h2 className="text-3xl font-black text-slate-900 dark:text-white">{stats.totalEarnings} BDT</h2>
                                <p className="text-xs font-semibold text-[#79a603] bg-[#79a603]/10 px-2 py-0.5 rounded-md inline-block mt-1">
                                ...
                                <AreaChart data={chartData}>
                                    <defs>
                                        <linearGradient id="colorSales" x1="0" y1="0" x2="0" y2="1">
                                            <stop offset="5%" stopColor="#79a603" stopOpacity={0.3}/>
                                            <stop offset="95%" stopColor="#79a603" stopOpacity={0}/>
                                        </linearGradient>
                                    </defs>
                                    <Area type="monotone" dataKey="sales" stroke="#79a603" strokeWidth={2.5} fillOpacity={1} fill="url(#colorSales)" />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    </Card>
                                        ...
                                        <td className="py-3.5 text-center">
                                            <span className={`inline-block px-2.5 py-0.5 rounded-full text-[11px] font-bold border
                                                ${product.availability === 'Available' 
                                                    ? 'bg-[#79a603]/10 text-[#79a603] border-[#79a603]/20' 
